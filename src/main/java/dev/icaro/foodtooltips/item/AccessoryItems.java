 ...
 * #fallDamageReductionPercent} are summed across every currently stored accessory (see
 * {@code AccessoryBagListener#fall}).
 */
public final class AccessoryItems {
     ...
     * for the Feather Talisman → Ring → Artifact line - lowercase by convention, but never
     * parsed as an enum so any future Collection can introduce its own without touching this
     * class), granting {@code fallHeightBonus} extra blocks of fall-damage-free height and
     * {@code fallDamageReductionPercent}% off whatever fall damage still applies past that -
     * called once while building the item, before {@code ItemStack#setItemMeta}.
     */
    public static void mark(ItemMeta meta, AccessoryType type, String family, int fallHeightBonus, double fallDamageReductionPercent) {
