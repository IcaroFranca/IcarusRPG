...
public final class BestiaryCatalog {
    ...
    private static BestiaryEntry e(EntityType type, Material icon, int combatXp, String orbXp, String ... drops) {
        return new BestiaryEntry(type.key().value(), type, icon, combatXp, orbXp, List.of(drops), null);
    }

    private static BestiaryEntry variant(String id, EntityType displayType, Material icon, String customName, int combatXp, String orbXp, String ... drops) {
        return new BestiaryEntry(id, displayType, icon, combatXp, orbXp, List.of(drops), customName);
    }
    ...
    /** Canonical (non-variant) lookup by raw EntityType - a variant entry never matches here, so a real Player death (or any type a variant reuses) can't be confused with it. */
    public static Optional<BestiaryEntry> find(EntityType type) {
        return ENTRIES.stream().filter(e -> e.type() == type && e.id().equals(type.key().value())).findFirst();
    }
    ...
    /** Checks the entity's {@link #VARIANT_KEY} tag first, falling back to its raw EntityType. */
    public static Optional<BestiaryEntry> find(Entity entity) {
        String variantId = entity.getPersistentDataContainer().get(VARIANT_KEY, PersistentDataType.STRING);
        if (variantId != null) {
            Optional<BestiaryEntry> byId = findById(variantId);
            if (byId.isPresent()) {
                return byId;
            }
        }
